...
|              策略              |      触发时机      |                       工作方式                        |            优点            |               缺点                |
|:----------------------------:|:--------------:|:-------------------------------------------------:|:------------------------:|:-------------------------------:|
|   惰性删除 (Lazy Expiration)		   |   客户端访问这个键时	   | 在执行任何命令访问键之前，Redis 会先检查其是否过期。若过期，则立即删除，并返回 (nil)	 |  CPU 友好：仅在被访问时才付出删除成本。	  | 内存不友好：大量过期键未被访问时，会一直占据内存（内存泄漏）。 |
| 定期删除 (Periodic Expiration)		 | Redis 主线程周期性执行 | 	每隔一段时间 (hz 配置决定频率)，随机抽取一批设置了 TTL 的键，删除其中已过期的键。	  | 	能够清理大量不被访问的过期键。减少内存浪费。	 | 	每次执行占用一定 CPU 时间。不是全量扫描，可能有残留。  |

### 惰性删除

“用到再检查”：超市中顾客拿起一瓶酸奶（访问键），收银员（Redis）会先检查保质期（是否过期）。过期了就不卖并处理掉（删除），不过期才正常出售（执行命令）。

#### 流程

1. 客户端执行一个**访问键**的操作（GET， HGET， EXISTS，甚至 *SCAN 命令实际访问时）。
2. Redis 在处理命令逻辑 之前，先检查该键是否设置了过期时间 (expires 字典中存在)。
3. 若设置了过期时间，检查当前时间是否大于该键的过期时间戳。
4. 如果过期：
    1. 删除该键。
    2. 不再执行客户端请求的命令逻辑。
    3. 返回 (nil) 或其他表示不存在的响应给客户端。

#### 优劣

🔴所有删除操作的成本都分摊在访问该键的客户端请求上。Redis 本身没有额外的后台检查开销。

⚫内存泄漏：过期键如果不被访问，就永远不会被删除。 即使已过期很久，仍会占据内存。

### 定期删除

“主动轮询，抽样清除”：超市的清洁工（定期删除）。他不检查每个商品（成本太高），而是巡逻定时几个货架区（数据库），随机抽查一些商品（随机键）
检查保质期（是否过期）。如果他发现某排货架过期品比例很高（比如抽查 20 个商品有 5 个过期了），他可能会在该区域
多停留检查一会儿（自适应调整）；如果没怎么发现过期品，他就按时下班（本次任务结束）。他的工作时间有上限（CPU 预算），不能耽误白天营业（主线程性能）。

#### 流程

1. Redis 的主事件循环 (serverCron 函数) 会每秒运行 server.hz 次 (默认 10 次，即每 100ms 运行一次，可配置)。其中的一项重要任务就是执行
   **定期过期键删除**。
2. 决定工作量：
    1. Redis 每次定期删除操作都设置了 时间上限（通常不超过 CPU 时间的 25%）。
    2. 目标是既清除足够多的过期键，又避免长时间阻塞主线程影响性能。
3. 随机取样 + 循环检查：
    1. 每次运行，Redis 会 随机选择一定数量的数据库 (db)，并在每个选中的数据库中 随机抽取一批设置了过期时间的键 (expires
       字典)。
    2. 这个抽取数量（ACTIVE_EXPIRE_CYCLE_LOOKUPS_PER_LOOP）是可配置的（通常与 hz 或动态计算相关），Redis
       会根据之前的清理效果自适应调整下次运行的采样数量和处理的数据库数量。
4. 删除过期键：
    1. 对于每次随机抽取到的键，检查其是否过期。如果过期，则删除。
    2. 如果一次运行结束时，删除的过期键数量达到了采样数的25%，并且还有未处理的数据库或键，Redis 会判断 “过期键还很多”，将在当前剩余的
       CPU 预算内立即开始下一轮随机抽样删除（在当前 serverCron 周期内继续进行），尽可能多地清理。
    3. 如果不到 25% 的过期率，或者预算时间用完，则本次运行结束，等待下次。

## 内存淘汰策略

客户端执行了一个需要分配新内存的写命令，同时在配置了 maxmemory 且已使用内存达到了 maxmemory 的情况下，根据
maxmemory-policy 删除键来为写命令腾出空间的保护策略。
...
| 策略                                         | 机制                                                                                                                                   | 优点                                       | 缺点                   | 适用场景                                          |
|--------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|----------------------|-----------------------------------------------|
| **allkeys-lru**<br>(Least Recently Used)   | Redis 维护一个**近似 LRU 算法**：<br>1. 每次访问键时更新其时间戳（非精确）。<br>2. 淘汰时，从全局随机抽取一批样本键(默认 5 个)。<br>3. 从样本中淘汰**访问时间戳最旧**的键。<br>4. 如果空间不够，重复步骤 2-3。	 | 实现简单<br>较好反映**访问热度**                     | 非精确LRU<br>扫描模式可能误伤热点 | 通用缓存场景<br>（页面/会话/热点数据缓存）                      |
| **allkeys-lfu**<br>(Least Frequently Used) | Redis 维护键的**访问频率计数器**：<br>1. 访问次数会随时间衰减（模拟窗口）。<br>2. 淘汰时，从全局随机抽取一批样本键。<br>3. 从样本中淘汰**访问频率计数器值最低**的键。<br>4. 如果空间不够，重复步骤 2-3。	         | 比 LRU 更能识别 长期访问频率。<br>对**周期性或突发访问**更强壮。	 | 非精确LFU<br>计算开销略高     | 对访问频率稳定性要求高、需要更精准淘汰低频数据的场景。<br>（用户兴趣标签/长期排行榜） |
| **allkeys-random**                         | 全局随机选择键删除                                                                                                                            | 实现最简单<br>零计算开销                           | 完全无策略<br>可能误删热点      | ⚠️ 仅测试或特殊需求<br>生产环境不推荐                        |

#### volatile-* (仅从设置了过期时间的键中淘汰)

| 策略                  | 机制                  | 优点             | 缺点                  | 适用场景                |
|---------------------|---------------------|----------------|---------------------|---------------------|
| **volatile-lru**    | 仅从设置了过期时间的键中执行LRU淘汰 | 保护永久数据         | TTL键少时选择有限          | 混合数据场景<br>（重要数据+缓存） |
| **volatile-lfu**    | 仅从设置了过期时间的键中执行LFU淘汰 | 精准淘汰低频缓存       | 同volatile-lru       | 混合数据+精准淘汰需求         |
| **volatile-ttl**    | 淘汰剩余过期时间最短的键        | 快速释放空间<br>实现简单 | 忽略访问热度<br>TTL均匀时效果差 | 优先淘汰即将过期数据          |
| **volatile-random** | 随机淘汰设置了过期时间的键       | 实现最简单          | 完全无策略               | ⚠️ 生产环境慎用           |

#### noeviction（拒绝写操作）

| 策略             | 行为                  | 风险             | 使用场景                |
|----------------|---------------------|----------------|---------------------|
| **noeviction** | 拒绝所有写入操作<br>返回OOM错误 | 服务不可写<br>影响可用性 | ❗ 生产环境禁用<br>仅特殊只读场景 |

#### 选型

1. 数据性质：
    - 所有数据都是缓存 (可丢)： 优先 allkeys-lru 或 allkeys-lfu。推荐 allkeys-lru。
    - 混合数据 (重要数据 + 缓存)： 重要数据不设置 TTL，缓存数据设置 TTL，选择 volatile-lru 或 volatile-lfu。推荐
      volatile-lru。
2. 访问模式：
    - 访问有明显热点 (最近访问)：*-lru。
    - 访问更看重长期频率：*-lfu。
    - 访问模式未知或复杂：*-lru 通常更强壮。
3. TTL 特点：
    - TTL 差异大且想快速腾空间：考虑 volatile-ttl。
4. 精准性 vs 开销：
    - 需要较好平衡：*-lru。
    - 追求更高精度（可接受略高开销）：*-lfu。
    - 完全不关心淘汰谁（仅需释放空间）：*-random (不推荐)。
...
缓存穿透指访问既不在缓存也不在数据库的数据，导致请求直接穿透缓存层持续轰炸数据库。

#### 防御策略

1. 非法请求拦截
    - 在请求入口处建立防线，过滤明确无效的请求（如负数的 ID、非法的字符组合）
    - 使用正则表达式验证参数格式有效性
2. 布隆过滤器防护
    - 在缓存层前方部署布隆过滤器
    - 所有合法数据在写入数据库时注册到过滤器中
    - 请求先经过过滤器校验：
        - 明确不存在 → 直接返回空
        - 可能存在 → 继续访问缓存
3. 空值缓存机制
    - 对数据库查询结果为空的情况，在缓存中存储特殊标记（如 "NULL"）
    - 设置较短过期时间（30-60 秒）
    - 后续相同请求直接命中空值标记
4. 用户行为分析
    - 监控频繁访问不存在 key 的客户端 IP
    - 自动屏蔽恶意 IP 地址
    - 实施请求速率限制（如单 IP 每秒最多 10 次查询）

### 缓存击穿（Cache Breakdown）

热点 Key 突然失效导致大量并发请求同时涌入数据库，造成数据库瞬时压力过载。

#### 防御策略

1. 缓存数据策略
    - 主动续期机制： 在缓存过期前提前更新（如到期前 5 分钟触发重建）
    - 逻辑过期设计：
        - 在缓存值中存储逻辑过期时间戳
        - 物理上永不过期
        - 应用层检查逻辑过期状态
2. 重建控制方案
    ...
    - 后台刷新策略：
        ...
        3. 重建完成后更新缓存值
3. 流量控制措施
    - 熔断降级机制： 当缓存失效时，若并发超过阈值，直接返回默认值
    - 请求合并技术： 将多个相同 Key 的请求合并为单个数据库查询

### 缓存雪崩（Cache Avalanche）

大量 Key 同时失效导致请求洪峰压垮数据库。

#### 防御策略

1. 缓存层策略
    - TTL 离散技术： 基础 TTL 值 + 随机偏移（如 3600±300 秒）
        - 均匀分布策略：平均分散各 Key 的过期时间
        - 分段分布策略：按业务模块设置不同的基础 TTL
    - 分级缓存体系：
        - L1：本地内存缓存（5-10 秒超短 TTL）
        - L2：Redis 集群缓存（主要缓存层）
        - L3：持久化存储（最后防线）
2. 架构层防护
    - 多可用区部署： 不同分区的缓存配置独立过期策略
    - 服务降级方案：
        - 缓存未命中时返回基础数据
        - 提供只读模式（维护期间）
        - 关键功能与非关键功能隔离
