...
- 层式结构：Dockerfile 中每条产生文件变更的指令（如 RUN, COPY, ADD）均生成堆叠在镜像(Image)的只读层的新层。
- 联合挂载：运行时通过联合文件系统（如 Overlay2）将镜像的只读层与容器的可写层合并为单一视图，容器进程感知完整的文件系统。
...
- ONBUILD(下游镜像触发器)：在当前镜像被用作另一个 Dockerfile 的 FROM 基础镜像时，自动在构建下游镜像时触发特定的指令
    ...
    - 示例（创建一个 Node 应用基础镜像）
      ...
      ```
    - 注意事项
        - 会导致构建行为更隐晦（用户可能不了解 ONBUILD 做了什么）。文档务必清晰。
        - 过度使用会使基础镜像不灵活。
...
- 静态模板与只读层: Docker 镜像是一个只读模板（Read-Only Template），它包含了运行一个应用所需的一切：操作系统基础（文件系统）、运行时环境（如
  Python、Node.js、JDK）、应用代码、库、环境变量和配置文件等。它由一系列只读层（Layers） 按照特定的顺序堆叠组合而成。
- 容器的基础： 镜像是创建 Docker 容器（Container） 的基础。当你运行一个容器时（docker run），Docker
  引擎会基于该镜像创建一个可写的容器层（Container Layer） 叠加在这些只读层之上。运行时所有的修改都只发生在这个可写层中。
  容器 = 镜像（只读层） + 可写层。

#### 关键特性

- 不可变性 (Immutability): 镜像本身一旦构建完成就是只读的、不可变的。这意味着无法直接修改一个已存在的镜像。如果需要更新应用或配置，必须重新构建一个新的镜像版本。
- 内容寻址 (Content-Addressable): 每个镜像层都有一个唯一的加密散列值（如 sha256:ab12cd...），基于其内容计算得出。这保证了镜像内容的完整性和唯一性。
- 分层存储 (Layered Storage): 镜像采用联合文件系统（Union File Systems, 如 Overlay2）
  实现分层存储。每个镜像层都是一个只读的文件系统快照，层与层之间通过联合挂载（Union Mount） 组合在一起，形成一个完整的文件系统视图。

#### Dockerfile 与 镜像

执行 docker build 命令时，Docker 引擎读取 Dockerfile 并按照其指令一步步执行，最终生成一个全新的 Docker 镜像。Dockerfile
是构建镜像的唯一标准输入。Dockerfile 是制造 Image 的配方和操作手册。docker build 就是执行这个手册的自动化生产线。

**But How?docker build 时发生了什么？**

1. 发送构建上下文： Docker CLI 将构建上下文（通常是包含 Dockerfile 的目录及其子目录）中的文件（过滤掉 .dockerignore
   中的内容）发送给 Docker 守护进程（daemon）。
2. 解析指令： Daemon 逐行解析 Dockerfile 指令。
3. 分层执行： 对于每一条能产生新层的指令（如 RUN, COPY, ADD）：
    1. Daemon 创建一个临时的中间容器来执行该指令。
    2. 指令执行完成后，Daemon 将该临时容器的文件系统变更（相对于上一层）提交为一个新的只读镜像层。
    3. 销毁临时容器。
    4. 使用缓存机制：如果当前指令及之前的指令、构建上下文内容与之前构建的缓存匹配，则直接复用已有的层，跳过执行。
4. 元数据设置： 对于不产生新文件系统层但影响镜像行为的指令（如 ENV, EXPOSE, CMD, ENTRYPOINT, LABEL, VOLUME），Daemon
   将这些信息作为镜像元数据存储在最终镜像的配置中。
5. 输出镜像： 当所有指令执行完毕，Daemon 将所有这些只读层和最终的镜像配置（含元数据） 打包组合成一个新的、可识别的 Docker
   镜像，并为其分配一个 标识符（通常是 [repository]:[tag] 如 myapp:v1，以及内部的 IMAGE ID）。
6. 多阶段构建处理： 如果是多阶段构建（Dockerfile 中有多个 FROM 语句），每个阶段的构建产物（层）是独立的。后续阶段可以COPY
   --from前一阶段的特定文件。只有最终阶段的层和配置会出现在输出镜像中。
